...
<html lang="zh-CN">
	...
	<body>
		...
		<div class="formbody">
		    <div id="usual1" class="usual"> 
			    <div class="itab">
				  	<ul> 
					    <li><a href="#tab1" class="selected">待审核</a></li> 
						...
						<tr class="formtabletr">
							...
							<td style="width: 18%">
								<select class="select" id="sfrzP" name="sfrzP" style="width: 70px">
									<option value="2" selected>已入账</option>
				...
				<div id="tab2" class="tabson">
					<table style="margin: 0px; width: 100%; border-bottom: solid 1px #ced9df;; border-right: solid 1px #ced9df;; border-left: solid 1px #ced9df;">
						<tr class="formtabletr">
							...
							<td style="width: 7%; text-align: center;">所属小区</td>
							<td style="width: 18%">
								<select name="xqC" id="xqC" class="select">
									<option value='' selected>请选择</option>
				            		<c:if test="${!empty communitys}">
										<c:forEach items="${communitys}" var="item">
											<option value='${item.key}'>${item.value.mc}</option>
						...
						<tr class="formtabletr">
							...
							<td style="width: 7%; text-align: center;">发生额</td>
							<td><input name="amountC" id="amountC" type="text" class="dfinput" value="" style="width: 202px;"/></td>
						</tr>
						<tr style="display:none">
							<td style="width: 7%; text-align: center;">选择年度</td>
							<td>
					...
					<div id="toolbar2" class="btn-group">
	   					...
	   					<button id="btn_add" type="button" class="btn btn-default" onclick="printData()">
	   						<span><img src='<c:url value='/images/t07.png'/>' /></span>打印凭证
	   					</button>
  					</div>
	...
	*/
		...
		function do_searchP() {
			$tableP.bootstrapTable('refresh',{url:"<c:url value='/vouchercheck/list'/>"});
		}

		function do_searchC() {
			$tableC.bootstrapTable('refresh',{url:"<c:url value='/vouchercheck/list'/>"});
		}
		// 导出待审核数据
		...
		function printData() {
			var p004s = "";
			// 循环选中列
			$($tableC.bootstrapTable('getSelections')).each(function(index, row){
				p004s += row.p004 + ",";
			});
            if(p004s == null || p004s == ""){ 
				art.dialog.alert("请先选中需要打印的数据！");
				return false;
            }else{
	            window.open("<c:url value='/vouchercheck/toPrint'/>?p004s="+p004s,'',
					'toolbar= no, menubar=yes, scrollbars=no, resizable=yes, location=no, status=no,top=10,left=30');
	            return false;
            }
		}
		...
		function onDblClick(row, $element) {
			var bm = "";
			var p004 = row.p004;
			var p005 = row.p005 == null? "": row.p005;
			if(p005 != "") {
				bm = p005;
				cxlb = "1";
			} else {
