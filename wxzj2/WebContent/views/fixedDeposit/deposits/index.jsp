...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		// 查询方法
		function do_search() {
			$table.bootstrapTable('refresh',{url:"<c:url value='/deposits/list'/>"});
		}

		// 新增方法
		function toAdd(){
			// 跳转页面
			window.location.href="<c:url value='/deposits/toAdd'/>";
		}
		
		// 删除方法(可用ajax方式提交请求，也可用跳转)
		function del(id) {
			art.dialog.confirm('是否删除当前记录?',function(){                       
				var url = "<c:url value='/deposits/delete?id='/>"+id;
		        location.href = url;
	    	});
		}
		...
		function edit(id) {
			window.location.href="<c:url value='/deposits/toUpdate'/>?id="+id;
		}
