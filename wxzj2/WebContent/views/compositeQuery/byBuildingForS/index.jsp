...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		function do_search() {
			...
			$table.bootstrapTable('refresh',{url:"<c:url value='/byBuildingForS/list'/>"});
		}
		...
		function exportData(){
			...
			lybh = $.trim($("#lybh").val());
			lymc = $("#lybh").find("option:selected").text();
        	begindate= $("#begindate").val();
        	enddate= $("#enddate").val();
        	pzsh=$("#zt").val();
        	cxlb=$("#cxfs").val();
            ...
            var param = "lybh="+lybh+"&begindate="+begindate+"&enddate="+enddate+"&pzsh="+pzsh+"&cxlb="+cxlb+"&lymc="+escape(escape(lymc))+"&xssy"+xssy;

            art.dialog.open(webPath + '/byBuildingForS/open/choosePrint' , {
    			id : 'choosePrint',
    			...
    			close : function() {
    				...
    				if (isClose == 0) {
    					...
    					//alert(items);
        	       		window.location.href="<c:url value='/byBuildingForS/exportByBuildingForS?'/>"+param;
    				}
    			}
    		});
		}
