...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		function do_search() {
			...
			lybh = $("#lybh").val() == null? "": $("#lybh").val();
			if(xqbh == "" && lybh == "") {
				art.dialog.alert("所属小区和所属楼宇不能都为空，请选择！");
				return;
			}
			$table.bootstrapTable('refresh',{url:"<c:url value='/queryPaymentS/list'/>"});
		}
		...
		function exportData(){
            ...
            var param = "lybh="+lybh+"&xqbh="+xqbh+"&begindate="+begindate+"&enddate="+enddate;
            window.location.href="<c:url value='/queryPaymentS/exportQueryPaymentS?'/>"+param;
		}
		...
		function toPrint() {
            ...
            enddate= $("#enddate").val();
            xqmc= $.trim($("#xqmc").val());
            if(xqbh != "") xqmc = $("#xqbh").find("option:selected").text();

            var param = "lybh="+lybh+"&xqbh="+xqbh+"&begindate="+begindate+"&enddate="+enddate+"&xqmc="+escape(escape(xqmc));
            var url = "<c:url value='/queryPaymentS/pdfQueryPaymentS?'/>"+param;
