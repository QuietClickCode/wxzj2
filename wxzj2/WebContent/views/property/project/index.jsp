...
<html lang="zh-CN">
			...
			<form action="" method="post" id="myForm">
				<table style="margin: 0; width: 100%; border: solid 1px #ced9df">
					<tr class="formtabletr">
						...
						<td style="width: 7%; text-align: center;">项目地址</td>
						<td style="width: 18%">
	...
	<script type="text/javascript">
		    ...
		    oTableInit.Init = function () {
		    	$(function () {
		    		$table.bootstrapTable({
						url: "<c:url value='/project/list'/>",  // 请求后台的URL（*）
		...
		// 跳转到新增页面
		function toAdd(){
			window.location.href="<c:url value='/project/toAdd'/>";
		}
		...
		// 编辑方法
		function edit(id) {
			var url = "<c:url value='/project/toUpdate?bm='/>"+id;
	        location.href = url;
		}
		...
		function del(id) {
			art.dialog.confirm('是否删除当前记录?',function(){                       
				var url = "<c:url value='/project/delete?bm='/>"+id;
		        location.href = url;
	    	});
		}
		...
		function batchDel() {
			...
			if (bms == null || bms == "") {
				...
				art.dialog.confirm('你确定要删除选中的'+i+'条数据吗？', function() {
					var url = "<c:url value='/project/batchDelete?bms='/>"+bms;
			        location.href = url;
				});
			}
		}
