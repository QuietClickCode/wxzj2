...
<html lang="zh-CN">
	...
	<body>
			...
			<form action="" method="post" id="myForm">
				<table style="margin: 0; width: 100%; border: solid 1px #ced9df">
					<tr class="formtabletr">
						<td style="width: 7%; text-align: center;">物业公司名称</td>
						<td style="width: 18%">
		    ...
		    oTableInit.Init = function () {
		    	$(function () {
		    		$table.bootstrapTable({
						url: "<c:url value='/propertycompany/list'/>",  //请求后台的URL（*）
						method: 'post',           //请求方式
		...
		function toAdd(){
			// 跳转页面
			window.location.href="<c:url value='/propertycompany/toAdd'/>";
		}
		// 删除方法(可用ajax方式提交请求，也可用跳转)
		function del(id) {
			art.dialog.confirm('是否删除当前记录?',function(){                       
				var url = "<c:url value='/propertycompany/delete?bm='/>"+id;
		        location.href = url;
	    	});
		}
		//批量删除
		function batchDel() {
			...
			if (bms == null || bms == "") {
				...
				art.dialog.confirm('你确定要删除选中的'+i+'条数据吗？', function() {
					var url = webPath+"propertycompany/batchDelete?bms="+bms;
			        location.href = url;
				});
			}
		}
		// 编辑方法
		function edit(bm) {
			window.location.href="<c:url value='/propertycompany/toUpdate'/>?bm="+bm;
		}
