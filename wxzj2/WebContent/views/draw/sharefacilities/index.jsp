...
<html lang="zh-CN">
			...
			<form action="<c:url value='/reviewal/list'/>" method="post" id="myForm">
				<table style="margin: 0; width: 100%; border: solid 1px #ced9df">
					<tr class="formtabletr">
		            	...
		            	<td style="width: 21%">
		            		<select name="bankCode" id="bankCode" class="chosen-select" style="width: 202px;">
	...
	<script type="text/javascript">
		    ...
		    oTableInit.Init = function () {
		    	$(function () {
		    		$table.bootstrapTable({
						url: "<c:url value='/sharefacilities/list'/>",  // 请求后台的URL（*）
		...
		function add(){
			// 跳转页面
			window.location.href="<c:url value='/sharefacilities/toAdd'/>";
		}
		...
		function del(){
			...
			if (bms == null || bms == "") {
				...
				art.dialog.confirm('你确定要删除选中的'+i+'条数据吗？', function() {
					var url = webPath+"sharefacilities/delshare?bms="+bms;
			        location.href = url;
				});
			}
		}
		...
		function printPdf(bm) {
			window.open("<c:url value='/sharefacilities/print'/>?bm="+bm,
		...
		function share(bm){
			// 跳转页面
			window.location.href="<c:url value='/sharefacilities/share'/>?bm="+bm;
		}
