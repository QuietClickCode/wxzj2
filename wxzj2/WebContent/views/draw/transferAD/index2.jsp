...
<html lang="zh-CN">
	...
	<body>
		...
		<div id="toolbar" class="btn-group">
	   		<button id="btn_add" type="button" class="btn btn-default" onclick="toTransferAD()">
	   			<span><img src="<c:url value='/images/leftico03.png'/>" /></span>划拨
	   		</button>
  		</div>
		...
		<div id="bankDiv" style="display: none;">
			<table>
				<tr>
					<th style="width: 80px;" >支取银行</th>
					<td>
						<select name="yhbh" id="yhbh" class="dfinput" style="width: 202px;" tabindex="1">
				...
				<tr>
					<th style="width: 80px;" >支取日期</th>
					<td>
						<input name="z003" id="z003" type="text" class="laydate-icon"
            				onclick="laydate({elem : '#z003',event : 'focus'});" 
	...
	<script type="text/javascript">
		    ...
		    oTableInit.Init = function () {
		    	$(function () {
		    		$table.bootstrapTable({
						url: "<c:url value='/transferAD/list'/>",  // 请求后台的URL（*）
						...
						},
						{
		                    ...
		                    align: 'center',
		                    formatter:function(value,row,index){  
				                //var e = '<a href="'+webPath+'/transferAD/transfer?bm='+ row.bm +'&z008='+row.zqbh+'" class="tablelink" mce_href="#">划拨</a>&nbsp;|&nbsp;'; 
				                var d = '<a href="#" class="tablelink" mce_href="#" onclick="delForTransferAD(\''+ row.bm + '\',\''+ row.zqbh + '\')">删除</a>'; 
		                    	if(zt=="7"){
					                return d ; 
				                }else{
					                return "" ;  
		...
		function toTransferAD(){
			...
			$($table.bootstrapTable('getSelections')).each(function(index, item){
				bms = item.bm+","+bms;
				z008s = $.trim(item.zqbh)+","+z008s;
				pzjes = item.pzje+","+pzjes;
				i++;
			});
			if(bms == ""){
				art.dialog.alert("请选中要划拨的数据！");
				return;
			}
			//alert(bms);
			//alert(z008s);
			//alert(pzjes);
			//return;
			 $("#z003").attr("value",getDate());
			 var content=$("#bankDiv").html();
			 art.dialog({                 
				id:'bankDiv',
				...
				parent: true
					...
					var z003 = $("#z003").val();
					var yhbh = $("#yhbh").val();
					var yhmc = $("#yhbh").find("option:selected").text();
					if(yhbh == ''){
						art.dialog.alert("请选择银行！");
						return false;
					}
					if(z003 == ''){
						art.dialog.alert("请选择支取日期！");
						return false;
					}
					art.dialog.confirm('是否确认选中的'+i+'个支取都是从'+yhmc+'划拨?',function(){
						$.ajax({  
			   				type: 'post',      
			   				url: webPath+"transferAD/saveTransferADMany",  
			   				data: {
				          		"z001" : z001,
				          		"z002" : z002,
				          		"z003" : z003,
			   				...
			   				success:function(data){ 
			   	                ...
			   	                if (result == null) {
			   	             	...
			   	             	if(result == 0) {
				                	art.dialog.succeed("划拨成功！");
				        			$table.bootstrapTable('refresh');
				                } else if(result == 2) {
				                	art.dialog.alert("业务：‘"+bm+"’的款项已经划拨，不允许重新划拨！");
				                } else if(result == 4) {
				                	art.dialog.alert("业务：‘"+bm+"’的发票还未启用，请检查！");
				                } else if(result == 5) {
				                	art.dialog.alert("业务：‘"+bm+"’的发票已用或者已作废，请检查！");
				                } else if(result == 6) {
				                	art.dialog.alert("业务：‘"+bm+"’的凭证审核失败，请手动审核 ！");
				                } else {
				                	art.dialog.error("业务：‘"+bm+"’保存失败，请稍候重试！");
				                }
			   				}
			            });
					});
        ...
        function isShowYLBL() {
      	...
      	//删除
      	function delForTransferAD(bm,zqbh) {
			art.dialog.confirm('你确定要删除申请编号为：'+bm+'的支取业务吗？',function(){
	          	$.ajax({  
	   				type: 'post',      
	   				url: webPath+"transferAD/delForTransferAD",  
	   				data: {
		          		"bm" : bm,
		          		"ywid" : zqbh
	   				},
	   				...
	   				success:function(result){ 
	   	                ...
	   	                if (result == null) {
		                	...
		                	art.dialog.alert("该记录已审核不允许删除，请检查！");
		                } else {
