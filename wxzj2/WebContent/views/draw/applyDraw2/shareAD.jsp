...
<html lang="zh-CN">
		...
		<div id="content">
				...
				<form action="<c:url value='/house/list'/>" method="post" id="myForm2">
					<table style="margin: 0; width: 100%; border: solid 1px #ced9df">
						...
						<tr class="formtabletr">
							<td style="width: 7%; text-align: center;">支取银行</td>
							<td style="width: 18%">
								<select name="yh" id="yh" class="dfinput" style="width: 202px;">
			...
			<div id="toolbar" class="btn-group">
	   		...
	   		<button id="btn_add" type="button" class="btn btn-default" onclick="printPdf()">
	   			<span><img src="<c:url value='/images/t07.png'/>"   width="24px;" height="24px;" /></span>打印清册
	   		</button>
	   		<button id="btn_add" type="button" class="btn btn-default" onclick="printPdfCollectsPay()">
	   			<span><img src="<c:url value='/images/t07.png'/>"   width="24px;" height="24px;" /></span>征缴打印
	   		</button>
  		</div>
			<div class="dataGrid"
	...
	<script type="text/javascript">
		...
		function do_submit_TJ() {
			...
			if(Number(hj_ftje) > ((Number(hj_kybj) + Number(hj_kylx)+Number(hj_zcje))*100/100)) {
        ...
        function do_submit() {
			...
			if(yftje>0){
				...
				//合计分摊金额
				var ftje=parseFloat(hj_ftje).toFixed(2);
				//合计可用金额加自筹金额
				var kyje=parseFloat(Number(hj_kybj) + Number(hj_kylx) + Number(hj_zcje)).toFixed(2);
				//差额
				var ce=ftje-kyje;
				if(ce>0) {
					alert("合计分摊金额不能大于合计可用金额加自筹金额，请检查后重试！");
			...
			var pzje = $("#show_bcpzje").val();
			var zqsj = $("#ftsj").val();

            art.dialog.tips("正在处理，请稍后…………",200000);
            $.ajax({  
   				type: 'post',      
   				url: webPath+"share/saveShareAD2",  
   				data: {
	                "bm" : bm,
	          		"type" : type,
	          		"yhbh" : yhbh,
		...
		function do_export(){
			...
			if(flag>0)
				return false;
			window.open(webPath+'share/exportShareAD?bm='+bm+'&lb='+lb+'&xqmc='+escape(escape(xqmc))+'','','toolbar= no, menubar=yes, scrollbars=no, resizable=yes, location=no, status=no,top=10,left=30');
		}
