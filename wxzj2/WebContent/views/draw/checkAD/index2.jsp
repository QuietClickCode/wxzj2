...
<html lang="zh-CN">
					...
					<tr class="formtabletr">
						<td style="width: 12%; text-align: center;">查询状态</td>
							...
							<select id="zt" name="zt" value="" class="select" >
									<option value="103" selected>审核</option>
	...
	<script type="text/javascript">
		    	...
		    	$(function () {
		    		$table.bootstrapTable({
						...
						},
						{
		                    ...
		                    formatter:function(value,row,index){  
								if($("#zt").val()=="2"){
									var a = '<a href="#" class="tablelink" mce_href="#" onclick="agree(\''+ row.bm + '\')">审核通过</a>&nbsp;|&nbsp;'; 
						 			var b = '<a href="#" class="tablelink" mce_href="#" onclick="returnCheckAD(\''+ row.bm + '\')">返回初审</a>'; 
									//var c = '<a href="#" class="tablelink" mce_href="#" onclick="search(\''+ row.bm + '\')">查看</a>'; 
									return a+b;
								}else{
					                var e = '<a href="#" class="tablelink" mce_href="#" onclick="agree(\''+ row.bm + '\')">审核通过</a>&nbsp;&nbsp;'; 
					                var d = '<a href="#" class="tablelink" mce_href="#" onclick="returnCheckAD(\''+ row.bm + '\')">返回初审</a>';  
					                return e + d;  
								}
		                	}
						}],
      ...
      //初审通过
      	function agree(bm) {
       		art.dialog.confirm('是否通过审核?',function(){
		       	art.dialog.data('flag','1');
       			art.dialog.data('bm',bm);
    			art.dialog.data('status','1');
      ...
      //返回申请
      	function returnCheckAD(bm) {
      		if(bm != "" && !isNaN(Number(bm))) {
      			art.dialog.confirm('是否确定将该申请退回？',function(){
	      			//传入隐藏的查询条件
	       			art.dialog.data('bm',bm);
	    			art.dialog.data('status','104');
      	        	art.dialog.data('isClose','1');
