...
<html lang="zh-CN">
					...
					<tr class="formtabletr">
						<td style="width: 12%; text-align: center;">查询状态</td>
							...
							<select id="zt" name="zt" value="" class="select" >
									<option value="1" selected>复核</option>
	...
	<script type="text/javascript">
		    	...
		    	$(function () {
		    		$table.bootstrapTable({
						...
						},
						{
		                    ...
		                    formatter:function(value,row,index){ 
								if($("#zt").val()=="4"){
									var a = '<a href="#" class="tablelink" mce_href="#" onclick="agree(\''+ row.bm + '\')">复核通过</a>&nbsp;|&nbsp;'; 
					                var b = '<a href="#" class="tablelink" mce_href="#" onclick="returnCheckAD(\''+ row.bm + '\')">返回审核</a>'; 
									//var c = '<a href="#" class="tablelink" mce_href="#" onclick="search(\''+ row.bm + '\')">查看</a>'; 
									return a+b;
								}else{ 
					                var e = '<a href="#" class="tablelink" mce_href="#" onclick="agree(\''+ row.bm + '\')">复核通过</a>&nbsp;&nbsp;'; 
					                var d = '<a href="#" class="tablelink" mce_href="#" onclick="returnCheckAD(\''+ row.bm + '\')">返回审核</a>';  
					                return e + d;  
								}
		                	}
						}],
        ...
        //通过复核
      	function agree(bm) {
       		art.dialog.confirm('是否通过复核?',function(){
		       	art.dialog.data('flag','1');
       			art.dialog.data('bm',bm);
    			art.dialog.data('status','3');
        ...
        //返回审核
      	function returnCheckAD(bm) {
      		if(bm != "" && !isNaN(Number(bm))) {
      			art.dialog.confirm('是否确定将该申请退回到审核?',function(){
	      			//传入隐藏的查询条件
	       			art.dialog.data('bm',bm);
	    			art.dialog.data('status','2');
      	        	art.dialog.data('isClose','1');
      	        	artDialog.open(webPath+'checkAD/toReturnReson?bm='+bm,{
      	            	id:'returnReson',
      	            	title: '退回到审核', //标题.默认:'提示'
      	            	top:30,
