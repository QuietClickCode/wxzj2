...
<html lang="zh-CN">
	...
	<body>
		...
		<div id="toolbar" class="btn-group">
	   		<button id="btn_add" type="button" class="btn btn-default" onclick="agreeChecked()">
	   			<span><img src="<c:url value='/images/btn/check.png'/>" width="24" height="24"/></span>审核通过
	   		</button>
  		</div>
	...
	<script type="text/javascript">
		    ...
		    oTableInit.Init = function () {
		    	$(function () {
		    		$table.bootstrapTable({
						url: "<c:url value='/reviewal/list'/>",  // 请求后台的URL（*）
		...
		function agreeChecked() {
			...
			if (bms == null || bms == "") {
				art.dialog.alert("请先选中要提交审核的数据！");
				return false;                                                                                                                               
			} else {
				art.dialog.confirm('你确定要将选中的'+i+'条数据审核通过吗？', function() {
					var url = webPath+"reviewal/agreeChecked?bms="+bms;
			        location.href = url;
				});
			}
		}
		...
		function returnCheck(bm){
        	...
        	artDialog.open('../views/draw/reviewal/returnreson.jsp',{                
            	id:'returnReson',
            	title: '返回申请', //标题.默认:'提示'
            	top:30,
            	...
            	close:function(){
                	...
                	if(isClose==0){  
            			...
            			var str=bm+";"+search.zt+";"+search.reason;
                   		art.dialog.confirm('是否将编号: '+bm+'申请数据退回？', function() {
            				var url = webPath+"reviewal/returnCheck?bms="+str;
            		        location.href = url;
            			});
                	}
           		}
