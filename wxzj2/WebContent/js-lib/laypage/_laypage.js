 ...
 * !
 * 
 * @Name : layPage v1.3- 分页插件 @Author: 贤心 @Site：http://sentsin.com/layui/laypage
 * @License：MIT
 */
...
!function() {
	...
	function laypage(options) {
		var skin = 'laypagecss';
		laypage.dir = 'dir' in laypage ? laypage.dir : Page.getpath
				+ '/skin/laypage.css';
		new Page(options);
	...
	Page.prototype.view = function() {
		...
		// 计算当前组
		dict.index = Math
				.ceil((conf.curr + ((conf.groups > 1 && conf.groups !== conf.pages)
						? 1
						: 0))
						/ (conf.groups === 0 ? 1 : conf.groups));

		// 当前页非首页，则输出上一页
		if (conf.curr > 1 && conf.prev) {
			view.push('<a href="javascript:;" class="laypage_prev" data-page="'
					+ (conf.curr - 1) + '">' + conf.prev + '</a>');
		}
		...
		for (; dict.start <= dict.end; dict.start++) {
			if (dict.start === conf.curr) {
				view.push('<span class="laypage_curr" '
						+ (/^#/.test(conf.skin) ? 'style="background-color:'
								+ conf.skin + '"' : '') + '>' + dict.start
						+ '</span>');
			} else {
				view.push('<a href="javascript:;" data-page="' + dict.start
						+ '">' + dict.start + '</a>');
			}
		}

		// 总页数大于连续分页数，且当前组最大页小于总页，输出尾页
		if (conf.pages > conf.groups && dict.end < conf.pages && conf.last
				&& conf.groups !== 0) {
			view
			...
			view.push((function() {
				return (dict.flow && conf.curr === conf.pages)
						...
						: '<a href="javascript:;" class="laypage_next" data-page="'
								+ (conf.curr + 1) + '">' + conf.next + '</a>';
			}()));
		}
		view.push('<span>共' + conf.pages + '页，' + conf.totalData + '条数据，');
		var everyPage = conf.everyPage;// 每页数据量
		var everyPageTmp = '每页'
				+ '<select name="everyPage" id="everyPage-layer" class="select1">';
		for (var i = 1; i < 11; i++) {
			if (i == everyPage / 10) {
				everyPageTmp += '<option value="' + i
						+ '0" selected="selected">' + i + '0</option>';
			} else {
				everyPageTmp += '<option value="' + i + '0">' + i
						+ '0</option>';
			}
		}
		everyPageTmp += '</select>条</span>';
		view.push(everyPageTmp);
		return '<div name="laypage' + laypage.v
				+ '" class="laypage_main laypageskin_'
				+ (conf.skin ? (function(skin) {
					return /^#/.test(skin) ? 'molv' : skin;
				}(conf.skin)) : 'default') + '" id="laypage_'
				+ that.config.item + '">' + view.join('') + function() {
					return conf.skip
							? '<span class="laypage_total"><label>到第</label><input type="number" value="'
									+ conf.curr
									+ '" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>页</label>'
