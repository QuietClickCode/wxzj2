...
(function($) {
	...
	_bindEvent = function(setting) {
		...
		o.unbind(c.NODECREATED).unbind(c.CLICK).unbind(c.EXPAND)
				...
				.unbind(c.ASYNC_ERROR).unbind(c.REMOVE).unbind(c.SELECTED)
				.unbind(c.UNSELECTED);
	},
		...
		onClickNode : function(event, node) {
			var setting = data.getSetting(event.data.treeId), clickFlag = ((setting.view.autoCancelSelected && (event.ctrlKey || event.metaKey)) && data
					...
					: (setting.view.autoCancelSelected
							&& (event.ctrlKey || event.metaKey) && setting.view.selectedMulti)
							? 2
							: 1;
	...
	view = {
		...
		makeDOMNodeIcon : function(html, setting, node) {
			...
			html.push("<span id='", node.tId, consts.id.ICON,
					...
					"' class='", consts.className.NAME, "'>", name, "</span>");
		},
...
/*
 * JQuery zTree excheck v3.5.24 http://zTree.me/
 * 
 * Copyright (c) 2010 Hunter.z
 * 
 * Licensed same as jquery - MIT License
...
(function($) {
	...
	_bindEvent = function(setting) {
		...
		o.bind(c.CHECK, function(event, srcEvent, treeId, node) {
					...
					tools
							.apply(setting.callback.onCheck, [event, treeId,
											node]);
				});
	}, _unbindEvent = function(setting) {
		var o = setting.treeObj, c = consts.event;
	...
	_zTreeTools = function(setting, zTreeTools) {
		zTreeTools.checkNode = function(node, checked, checkTypeFlag,
			...
			if (tools.uCanDo(this.setting) && this.setting.check.enable
				...
				if (callbackFlag) {
					this.setting.treeObj.trigger(consts.event.CHECK, [null,
		...
		zTreeTools.getCheckedNodes = function(checked) {
			...
			return data.getTreeCheckedNodes(this.setting, data
							.getRoot(this.setting)[childKey], checked);
		}

		zTreeTools.getChangeCheckedNodes = function() {
			var childKey = this.setting.data.key.children;
			return data.getTreeChangeCheckedNodes(this.setting, data
							.getRoot(this.setting)[childKey]);
		}
...
/*
 * JQuery zTree exedit v3.5.24 http://zTree.me/
 * 
 * Copyright (c) 2010 Hunter.z
 * 
 * Licensed same as jquery - MIT License
...
(function($) {
	...
	_bindEvent = function(setting) {
		...
		o.bind(c.DRAGMOVE, function(event, srcEvent, treeId, treeNodes) {
					tools.apply(setting.callback.onDragMove, [srcEvent, treeId,
									treeNodes]);
				});
	...
	_zTreeTools = function(setting, zTreeTools) {
		zTreeTools.cancelEditName = function(newName) {
			...
			if (!root.curEditNode)
				return;
			view
					.cancelCurEditNode(this.setting, newName ? newName : null,
							true);
		}
		zTreeTools.copyNode = function(targetNode, node, moveType, isSilent) {
			...
			if (moveType == consts.move.TYPE_INNER) {
				function copyCallback() {
					view.addNodes(_this.setting, targetNode, -1, [newNode],
							isSilent);
				}

				if (tools.canAsync(this.setting, targetNode)) {
					view.asyncNode(this.setting, targetNode, isSilent,
							copyCallback);
				} else {
					copyCallback();
				}
				...
				view.addNodes(this.setting, targetNode.parentNode, -1,
						[newNode], isSilent);
				view.moveNode(this.setting, targetNode, newNode, moveType,
						false, isSilent);
			}
			return newNode;
		}
		...
		zTreeTools.moveNode = function(targetNode, node, moveType, isSilent) {
			...
			if (tools.canAsync(this.setting, targetNode)
					&& moveType === consts.move.TYPE_INNER) {
				view
						.asyncNode(this.setting, targetNode, isSilent,
								moveCallback);
			} else {
				moveCallback();
			}
		...
		onMousedownNode : function(eventMouseDown, _node) {
			...
			function _docMouseMove(event) {
				...
				if (root.dragFlag == 1) {
					...
					if (event.target
							&& tools.isChildOrSelf(event.target,
					...
					if (tmpTarget
						...
						var tmpTargetNode = tmpTargetNodeId == null
								...
								: data.getNodeCache(targetSetting,
										tmpTargetNodeId), isCopy = ((event.ctrlKey || event.metaKey)
	...
	_tools = {
		getAbs : function(obj) {
			var oRect = obj.getBoundingClientRect(), scrollTop = document.body.scrollTop
					+ document.documentElement.scrollTop, scrollLeft = document.body.scrollLeft
		...
		cancelCurEditNode : function(setting, forceName, isCancel) {
			...
			if (node) {
				var inputObj = root.curEditInput, newName = forceName
						? forceName
						: (isCancel ? node[nameKey] : inputObj.val());
				...
				setting.treeObj.trigger(consts.event.RENAME, [setting.treeId,
								node, isCancel]);
			}
