...
public class TransferADPDF {
	...
	static Logger logger = LoggerFactory.getLogger(TransferADPDF.class);
	...
	public void creatPDF(ApplyDraw applyDraw,Double bcsqje,String title,HttpServletResponse rep) throws Exception {
		init();
		response = rep;
		...
		buffer.append(applyDraw.getNbhdcode()).append("(").
				append(applyDraw.getNbhdname()).append(")");
		if(buffer.toString().trim().equals("()")){
			buffer = new StringBuffer("    ");
			buffer.append(applyDraw.getXmmc());
		}
		if (!applyDraw.getBldgcode().equals("")) {
			buffer.append("/").append(applyDraw.getBldgcode()).
					append("(").append(applyDraw.getBldgname()).append(")");
		}
		...
		cell = PdfUtil.createCell("划拨金额", headFont2, 1, 20, 1, PdfUtil.CENTER_H,
		...
		cell = PdfUtil.createCell(ChangeRMB.doChangeRMB(df.format(bcsqje)), headFont2, 1, 40, 2, PdfUtil.LEFT_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		String[] sqje = PdfUtil.convert(df.format(bcsqje));
		for (int i = 0; i < str.length; i++) {
		...
		table.addCell(cell);
		// ----------------------
		cell = PdfUtil.createCell("支取原因", headFont2, 1, 20, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		cell = PdfUtil.createCell("申请单位盖章", headFont2, 1, 25, 1, PdfUtil.CENTER_H,
				PdfUtil.BOTTOM_V);
		cell.setBorder(1);
		cell.setBorderWidthBottom(0);
		cell.setBorderWidthTop(0);
