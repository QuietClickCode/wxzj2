...
/**
 * 
 * @ClassName: QueryAL1PDF
 * @Description: 明细查询清册打印
 * 
 * @author yangshanping
 * @date 2016-10-9 下午02:22:11
 */
public class QueryAL1PDF extends AbstractPDFService{
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		super.init(model);
		// 获取传入参数
		List<QueryAL1> list = getValue("list", List.class); // 房屋信息
		...
		float[] widths = { 80f, 30f, 30f, 40f, 80f, 60f, 60f, 80f };// 设置表格的列以及列宽
		...
		PdfPCell cell = PdfUtil.createCell("销户支取明细清册", headFont1, 0,
		...
		table.addCell(cell);
		cell = PdfUtil.createCell("支取本金", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		cell = PdfUtil.createCell("支取利息", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getZ012(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(String.valueOf(list.get(i).getZ004()), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(String.valueOf(list.get(i).getZ005()), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
