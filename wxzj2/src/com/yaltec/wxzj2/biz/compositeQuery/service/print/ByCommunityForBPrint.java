...
public class ByCommunityForBPrint extends AbstractPDFService {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		super.init(model);
		// 获取传入参数
		List<ByCommunityForB> list = getValue("list", List.class); // 小区余额查询信息
		...
		PdfPCell cell = PdfUtil.createCell("小区余额查询结果", headFont1, 0, 55, widths.length, PdfUtil.CENTER_H,
				PdfUtil.BOTTOM_V);
		...
		// -------------
		// 打印显示列名
		cell = PdfUtil.createCell("小区编号", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		for (int i = 0; i < list.size(); i++) {
			cell = PdfUtil.createCell(list.get(i).getXqbh(), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
