...
public class BuildingInterestFPrint extends AbstractPDFService{
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		super.init(model);
		// 获取传入参数
		List<BuildingInterestF> list = getValue("list", List.class); // 楼宇利息单查询信息
		...
		float[] widths = { 80f, 130f, 80f, 150f };// 设置表格的列以及列宽
		...
		PdfPCell cell = PdfUtil.createCell("物业专项维修资金楼宇利息单打印清册", headFont1, 0,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getMc(), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getW006(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
