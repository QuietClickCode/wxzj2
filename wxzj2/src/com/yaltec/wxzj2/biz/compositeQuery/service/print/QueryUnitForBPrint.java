...
public class QueryUnitForBPrint extends AbstractPDFService {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		super.init(model);
		// 获取传入参数
		List<QueryUnitForB> list = getValue("list", List.class); // 单元余额查询结果信息
		...
		float[] widths = { 50f, 120f, 60f, 60f, 60f, 60f, 60f, 60f, 60f};// 设置表格的列以及列宽
		...
		PdfPCell cell = PdfUtil.createCell("单元余额查询结果", headFont1, 0, 55, widths.length, PdfUtil.CENTER_H,
				PdfUtil.BOTTOM_V);
		...
		if (xqmc.equals("") || xqmc.equals("请选择")) {
			xqmc = "所有楼宇";
		}
		...
		table.addCell(cell);
		cell = PdfUtil.createCell("单元名称", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		cell = PdfUtil.createCell("交款金额", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		cell = PdfUtil.createCell("支出金额", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			table.addCell(cell);
			cell = PdfUtil.createCell(String.valueOf(list.get(i).getJkje()), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(String.valueOf(list.get(i).getZqje()), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(String.valueOf(list.get(i).getBj()), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil
					.createCell(String.valueOf(list.get(i).getLx()), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(String.valueOf(list.get(i).getYe()), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
