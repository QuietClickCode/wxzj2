...
public class ByProjectForBPrint extends AbstractPDFService {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		super.init(model);
		// 获取传入参数
		List<ByProjectForB> list = getValue("list", List.class); // 项目余额查询信息
		...
		float[] widths = { 80f, 130f, 60f, 60f, 60f, 60f, 60f, 80f };// 设置表格的列以及列宽
		...
		PdfPCell cell = PdfUtil.createCell("项目余额查询结果", headFont1, 0, 55, widths.length, PdfUtil.CENTER_H,
				PdfUtil.BOTTOM_V);
		table.addCell(cell);// 增加单元格

		// 空行(名称下面显示两条信息，左边显示所属小区，右边显示日期)
		...
		// -------------
		// 打印显示列名
		cell = PdfUtil.createCell("项目编号", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		table.addCell(cell);
		cell = PdfUtil.createCell("支取金额", headFont2, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
		table.addCell(cell);
		...
		for (int i = 0; i < list.size(); i++) {
			cell = PdfUtil.createCell(list.get(i).getXmbh(), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getXmmc(), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getJkje(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getZqje(), headFont3, 1, 25, 1, PdfUtil.CENTER_H, PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getBj(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getLx(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			cell = PdfUtil.createCell(list.get(i).getYe(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
					PdfUtil.MIDDLE_V);
			table.addCell(cell);
			...
			if (list.get(i).getMdate().length() > 10) {
				...
				cell = PdfUtil.createCell(list.get(i).getMdate(), headFont3, 1, 25, 1, PdfUtil.CENTER_H,
						PdfUtil.MIDDLE_V);
			}
			table.addCell(cell);
		}
