...
/**
 * 变更查询导出
 * @ClassName: ChangePropertyExport 
 * @author 重庆亚亮科技有限公司 hqx 
 * @date 2016-9-5 上午09:54:35
 */
public class ChangePropertyExport {
	...
	public static void exportChangeProperty(List<ChangeProperty> list,
			HttpServletResponse response) throws IOException {
		OutputStream os = setStream("产权变更信息", response);
		// 第一步，创建一个webbook，对应一个Excel文件
		HSSFWorkbook wb = new HSSFWorkbook();
		// 第二步，在webbook中添加一个sheet,对应Excel文件中的sheet
		HSSFSheet sheet = wb.createSheet("产权变更信息");
		...
		cell = row.createCell(4);
		cell.setCellValue("原业主姓名");
		cell.setCellStyle(style);

		cell = row.createCell(5);
		cell.setCellValue("现业主姓名");
		cell.setCellStyle(style);

		cell = row.createCell(6);
		cell.setCellValue("原房屋性质");
		cell.setCellStyle(style);

		cell = row.createCell(7);
		cell.setCellValue("现房屋性质");
		cell.setCellStyle(style);

		cell = row.createCell(8);
		cell.setCellValue("变更日期");
		cell.setCellStyle(style);

		cell = row.createCell(9);
		cell.setCellValue("卖方身份证号");
		cell.setCellStyle(style);
		
		cell = row.createCell(10);
		cell.setCellValue("买方身份证号");
		cell.setCellStyle(style);

		// 第五步，写入实体数据 实际应用中这些数据从数据库得到
		for (int i = 0; i < list.size(); i++) {
			row = sheet.createRow((int) i + 2);
			ChangeProperty ad = (ChangeProperty) list.get(i);
			...
			cell=row.createCell(4);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getO013()));
			
			
			cell=row.createCell(5);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getN013()));
			
			cell=row.createCell(6);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getO012()));
			
			cell=row.createCell(7);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getN012()));
			
			cell=row.createCell(8);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getBgrq()));
			
			cell=row.createCell(9);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getO015()));
			
			cell=row.createCell(10);
			cell.setCellStyle(style1);
			cell.setCellValue(StringUtil.valueOf(ad.getN015()));
		}
		// 第六步，将文件存到指定位置
		try {
